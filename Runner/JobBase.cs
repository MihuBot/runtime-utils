...
public abstract class JobBase
{
    ...
    public async Task ZipAndUploadArtifactAsync(string zipFileName, string folderPath, bool maxCompression = false)
    {
        ...
        else
        {
            ...
            await RunProcessAsync("zip", $"-{(maxCompression ? 6 : 3)} -r {zipFilePath} .", logPrefix: zipFileName, workDir: folderPath, suppressOutputLogs: true);
        }
    ...
    private async Task<string?> ChangeWorkingDirectoryToRamOrFastestDiskAsyncCore(bool allowRamDisk)
    {
        ...
        if (TryGetFlag("helix") || Metadata.ContainsKey("HelixQueue"))
        {
            return null;
        }
